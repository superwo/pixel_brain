...
# ---------------- Main ----------------
def run(seed=3, H=160, W=160, T=2500,
        capture_every=6,
        out_gif="pixel_cell_m7_aggressive.gif"):
    ...
    for t in range(T):
        ...
        # ---------- RETINA (ACTIVE) ----------
        # Bias movement toward curiosity gradient
        ...
        # ---------- FUTURE PREDICTION ----------
        if len(world_buffer) > k_future:
            target = world_buffer[0]  # k steps ahead
            err = np.abs(P - target)
