...
# ==========================================================
# M12 FEATURES (TOGGLEABLE)
# ==========================================================
USE_DIFFERENTIATION = True
USE_SPATIAL_SPECIALIZATION = True
USE_STRUCTURED_CONNECTIVITY = True
USE_LONG_TERM_PLASTICITY = True
USE_HIERARCHY = True

# ==========================================================
# BIO PARAMETERS (UNCHANGED CORE)
# ==========================================================
BASE_ENERGY = 0.02
...
# M12 STRUCTURES
# ==========================================================
cell_type = np.zeros((H, W), dtype=np.int8)
# 0 = support, 1 = sensory, 2 = memory, 3 = output

spatial_bias = np.linspace(0, 1, H)[:, None]
...
def step():
    ...
    global X, M, E, I, S, alive, W_reg, cell_type

    # ---- Live delay switching (UNCHANGED) ----
    ...
    # ---- Gene regulation (CORE + ADDITIVE M12) ----
    new_X = np.zeros_like(X)

    for g in range(GENES):
        reg_input = np.tensordot(X, W_reg[:, g], axes=([2], [0]))

        # M12 structured connectivity
        if USE_STRUCTURED_CONNECTIVITY:
            reg_input += 0.08 * laplacian(X[..., g])

        # M12 spatial specialization
        if USE_SPATIAL_SPECIALIZATION:
            reg_input += 0.12 * spatial_bias
    ...
    if USE_DIFFERENTIATION:
        cell_type[(activity < 0.4) & alive] = 0
        cell_type[(activity >= 0.4) & (activity < 1.2) & alive] = 2
        cell_type[(activity >= 1.2) & alive] = 3
        cell_type[: H // 4][alive[: H // 4]] = 1
    ...
    if USE_HIERARCHY:
        top_mean = X[: H // 3].mean()
        mid_mean = X[H // 3 : 2 * H // 3].mean()
        bot_mean = X[2 * H // 3 :].mean()
        E += 0.05 * (mid_mean - bot_mean)

    # ---- Survival (UNCHANGED CORE) ----
    survival_score = (
    ...
    for i, j in zip(*np.where(grow)):
        ...
        for ni, nj in neighbors:
            if not alive[ni, nj]:
                ...
                E[ni, nj] = 0.5
                if USE_DIFFERENTIATION:
                    cell_type[ni, nj] = cell_type[i, j]
                break

    # ---- Plasticity (UNCHANGED CORE) ----
    ...
    # ---- M12 Long-Term Plasticity ----
    if USE_LONG_TERM_PLASTICITY:
        W_reg += 5e-6 * W_reg
        W_reg = np.clip(W_reg, -2, 2)
